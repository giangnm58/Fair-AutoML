...
class CustomGBC(
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter("n_estimators", 241, 813, default_value=241)
        loss = CategoricalHyperparameter(
            "loss", ["deviance", "exponential"], default_value="deviance")
        learning_rate = UniformFloatHyperparameter("learning_rate", 0.19922, 0.77579, default_value=0.19922)
        max_features = UniformFloatHyperparameter(
            "max_features", 0.22774, 0.83729, default_value=0.5)
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['nystroem_sampler', 'extra_trees_preproc_for_classification', 'pca'],
...
a_file = open("bank_gbc_di" + str(now) + "60sp.pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_bank_gbc_di" + str(now) + "60sp.pkl", "wb")
