...
now = now.replace(":","_")
temp_path = "bank_xgb7_aod" + str(now)
try:
...
class CustomXGBoost(AutoSklearnClassificationAlgorithm):
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter("n_estimators", 210, 767, default_value=210)
        max_depth = UniformIntegerHyperparameter("max_depth", 3, 9,
                                                         default_value=7)
        learning_rate = UniformFloatHyperparameter("learning_rate", 0.17415, 0.73086,
                                                        default_value=0.17415)
        subsample = UniformFloatHyperparameter("subsample", 0.24078, 0.76294,
                                                  default_value=0.75)
...
def accuracy(solution, prediction):
    ...
    if os.stat("beta.txt").st_size == 0:
        ...
        default = XGBClassifier(n_estimators=100, learning_rate=0.08, gamma=0, subsample=0.75,
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['select_percentile_classification', 'liblinear_svc_preprocessor', 'select_rates_classification'],
...
a_file = open("bank_xgb7_aod" + str(now) + "60sp.pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_bank_xgb7_aod" + str(now) + "60sp.pkl", "wb")
