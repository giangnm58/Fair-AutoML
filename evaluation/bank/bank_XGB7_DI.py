...
now = now.replace(":","_")
temp_path = "bank_xgb7_di" + str(now)
try:
...
class CustomXGBoost(AutoSklearnClassificationAlgorithm):
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter("n_estimators", 308, 806, default_value=308)
        max_depth = UniformIntegerHyperparameter("max_depth", 3, 9,
                                                 default_value=6)
        learning_rate = UniformFloatHyperparameter("learning_rate", 0.20693, 0.78709,
                                                   default_value=0.3)
        subsample = UniformFloatHyperparameter("subsample", 0.23764, 0.78685,
                                               default_value=0.78685)
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['fast_ica', 'select_percentile_classification', 'kitchen_sinks'],
...
a_file = open("bank_xgb7_di" + str(now) + "60sp.pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_bank_xgb7_di" + str(now) + "60sp.pkl", "wb")
