...
now = now.replace(":","_")
temp_path = "bank_gbc_eod" + str(now)
try:
...
class CustomGBC(
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter("n_estimators", 231, 755, default_value=231)
        loss = CategoricalHyperparameter(
            "loss", ["deviance", "exponential"], default_value="deviance")
        learning_rate = UniformFloatHyperparameter("learning_rate", 0.27244, 0.81732, default_value=0.27244)
        max_features = UniformFloatHyperparameter(
            "max_features", 0.20899, 0.82205, default_value=0.5)
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['nystroem_sampler', 'select_rates_classification', 'select_percentile_classification'],
...
a_file = open("bank_gbc_eod" + str(now) + "60sp.pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_bank_gbc_eod" + str(now) + "60sp.pkl", "wb")
