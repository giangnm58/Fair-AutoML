...
now = now.replace(":","_")
temp_path = "bank_xgb7_eod" + str(now)
try:
...
class CustomXGBoost(AutoSklearnClassificationAlgorithm):
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter("n_estimators", 212, 772, default_value=212)
        max_depth = UniformIntegerHyperparameter("max_depth", 3, 9,
                                                 default_value=6)
        learning_rate = UniformFloatHyperparameter("learning_rate", 0.20693, 0.72593,
                                                   default_value=0.3)
        subsample = UniformFloatHyperparameter("subsample", 0.18777, 0.77637,
                                               default_value=0.77637)
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['select_percentile_classification', 'fast_ica', 'extra_trees_preproc_for_classification'],
...
a_file = open("bank_xgb7_eod" + str(now) + "60sp.pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_bank_xgb7_eod" + str(now) + "60sp.pkl", "wb")
