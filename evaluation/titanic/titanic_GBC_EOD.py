...
now = now.replace(":", "_")
temp_path = "titanic_gbc_eod" + str(now)
try:
...
class CustomGBC(
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter("n_estimators", 206, 797, default_value=206)
        loss = CategoricalHyperparameter(
            "loss", ["deviance", "exponential"], default_value="deviance")
        learning_rate = UniformFloatHyperparameter("learning_rate", 0.16070, 0.71208, default_value=0.16070)
        max_features = UniformFloatHyperparameter(
            "max_features", 0.19602, 0.74589, default_value=0.5)
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['kernel_pca', 'nystroem_sampler', 'fast_ica'],
...
# ===================================
a_file = open("titanic_gbc_eod_60sp" + str(now) + ".pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_titanic_gbc_eod_60sp" + str(now) + ".pkl", "wb")
