...
for i in range(len(train_list)):
    ...
    now = now.replace(":", "_")
    temp_path = "titanic_lrg_di" + str(now)
    try:
    ...
    class CustomLRG(AutoSklearnClassificationAlgorithm):
        ...
        def get_hyperparameter_search_space(dataset_properties=None):
            ...
            solver = CategoricalHyperparameter(name='solver', choices=['newton-cg',  'liblinear', 'sag', 'saga'],
                                          default_value='liblinear')
            fit_intercept = CategoricalHyperparameter(name='fit_intercept', choices=['newton-cg',  'liblinear', 'sag', 'saga'],
                                          default_value='liblinear')
    ...
    automl = autosklearn.classification.AutoSklearnClassifier(
        ...
        ensemble_size=1,
        include_preprocessors=['extra_trees_preproc_for_classification', 'nystroem_sampler', 'select_percentile_classification'],
    ...
    # ===================================
    a_file = open("titanic_lrg_di_60sp" + str(now) + ".pkl", "wb")
    pickle.dump(automl.cv_results_, a_file)
    a_file.close()

    a_file1 = open("automl_titanic_lrg_di_60sp" + str(now) + ".pkl", "wb")
