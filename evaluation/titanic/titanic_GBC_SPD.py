...
now = now.replace(":", "_")
temp_path = "titanic_gbc_spd" + str(now)
try:
...
class CustomGBC(
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter("n_estimators", 177, 731, default_value=177)
        loss = CategoricalHyperparameter(
            "loss", ["deviance", "exponential"], default_value="deviance")
        learning_rate = UniformFloatHyperparameter("learning_rate", 0.20866, 0.74261, default_value=0.20866)
        max_features = UniformFloatHyperparameter(
            "max_features", 0.22631, 0.73291, default_value=0.5)
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['kernel_pca', 'select_rates_classification', 'fast_ica'],
...
# ===================================
a_file = open("titanic_gbc_spd_60sp" + str(now) + ".pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_titanic_gbc_spd_60sp" + str(now) + ".pkl", "wb")
