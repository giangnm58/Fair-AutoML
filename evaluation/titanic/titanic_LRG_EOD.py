...
for i in range(len(train_list)):
    ...
    now = now.replace(":", "_")
    temp_path = "titanic_lrg_eod" + str(now)
    try:
    ...
    automl = autosklearn.classification.AutoSklearnClassifier(
        ...
        ensemble_size=1,
        include_preprocessors=['extra_trees_preproc_for_classification', 'liblinear_svc_preprocessor', 'select_rates_classification'],
    ...
    # ===================================
    a_file = open("titanic_lrg_eod_60sp" + str(now) + ".pkl", "wb")
    pickle.dump(automl.cv_results_, a_file)
    a_file.close()

    a_file1 = open("automl_titanic_lrg_eod_60sp" + str(now) + ".pkl", "wb")
