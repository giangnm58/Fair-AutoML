...
now = now.replace(":", "_")
temp_path = "titanic_xgb_spd" + str(now)
try:
...
class CustomXGBoost(AutoSklearnClassificationAlgorithm):
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter("n_estimators", 182, 723, default_value=300)
        max_depth = UniformIntegerHyperparameter("max_depth", 3, 9,
                                                 default_value=4)
        learning_rate = UniformFloatHyperparameter("learning_rate", 0.08891, 0.61335,
                                                   default_value=0.08891)
        subsample = UniformFloatHyperparameter("subsample", 0.35198, 0.86236,
...
a_file1 = open("automl_titanic_xgb_spd_60sp" + str(now) + ".pkl", "wb")
