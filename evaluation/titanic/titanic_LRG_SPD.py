...
for i in range(len(train_list)):
    ...
    now = now.replace(":", "_")
    temp_path = "titanic_lrg_spd" + str(now)
    try:
    ...
    automl = autosklearn.classification.AutoSklearnClassifier(
        ...
        ensemble_size=1,
        include_preprocessors=['pca', 'extra_trees_preproc_for_classification', 'select_rates_classification'],
    ...
    # ===================================
    a_file = open("titanic_lrg_spd_60sp" + str(now) + ".pkl", "wb")
    pickle.dump(automl.cv_results_, a_file)
    a_file.close()

    a_file1 = open("automl_titanic_lrg_spd_60sp" + str(now) + ".pkl", "wb")
