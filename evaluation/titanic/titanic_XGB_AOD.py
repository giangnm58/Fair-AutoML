...
now = now.replace(":", "_")
temp_path = "titanic_xgb_aod" + str(now)
try:
...
class CustomXGBoost(AutoSklearnClassificationAlgorithm):
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter("n_estimators", 185, 771, default_value=300)
        max_depth = UniformIntegerHyperparameter("max_depth", 3, 9,
                                                 default_value=4)
        learning_rate = UniformFloatHyperparameter("learning_rate", 0.09640, 0.63767,
                                                   default_value=0.09640)
        subsample = UniformFloatHyperparameter("subsample", 0.40780, 0.897068,
                                               default_value=0.897068)
...
# ===================================
a_file = open("titanic_xgb_aod_60sp" + str(now) + ".pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_titanic_xgb_aod_60sp" + str(now) + ".pkl", "wb")
