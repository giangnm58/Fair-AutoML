...
train_list = "data_orig_train_adult1.pkl"
test_list = "data_orig_test_adult1.pkl"
...
now = now.replace(":","_")
temp_path = "adult_gbc_aod" + str(now)
try:
...
class CustomGBC(
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter("n_estimators", 250, 744, default_value=250)
        loss = CategoricalHyperparameter(
            "loss", ["deviance", "exponential"], default_value="deviance")
        learning_rate = UniformFloatHyperparameter("learning_rate", 0.25608, 0.78121, default_value=0.25608)
        max_features = UniformFloatHyperparameter(
            "max_features", 0.23027, 0.83967, default_value=0.5)
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['select_percentile_classification', 'select_rates_classification', 'nystroem_sampler'],
...
a_file = open("adult_gbc_aod_60sp" + str(now) + ".pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_adult_gbc_aod_60sp" + str(now) + ".pkl", "wb")
