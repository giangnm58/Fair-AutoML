...
class CustomRandomForest(AutoSklearnClassificationAlgorithm):
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter("n_estimators", 240, 781, default_value=240)
        ...
        max_features = UniformFloatHyperparameter(
            "max_features", 0.13434, 0.73101, default_value=0.5)

        max_depth = UnParametrizedHyperparameter("max_depth", "None")
        min_samples_split = UniformIntegerHyperparameter(
            "min_samples_split", 7, 16, default_value=7)
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['select_rates_classification', 'feature_agglomeration', 'fast_ica'],
...
import shutil
a_file = open("adult_rf_spd_60sp" + str(now) + ".pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_adult_rf_spd_60sp" + str(now) + ".pkl", "wb")
