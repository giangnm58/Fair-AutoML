...
now = now.replace(":","_")
temp_path = "adult_gbc_di" + str(now)
try:
...
class CustomGBC(
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter("n_estimators", 190, 727, default_value=190)
        loss = CategoricalHyperparameter(
            "loss", ["deviance", "exponential"], default_value="deviance")
        learning_rate = UniformFloatHyperparameter("learning_rate", 0.22824, 0.71029, default_value=0.22824)
        max_features = UniformFloatHyperparameter(
            "max_features", 0.24615, 0.79194, default_value=0.5)
...
a_file = open("adult_gbc_di_60sp" + str(now) + ".pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_adult_gbc_di_60sp" + str(now) + ".pkl", "wb")
