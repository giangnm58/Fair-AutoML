...
now = now.replace(":", "_")
temp_path = "adult_xgb_di" + str(now)
try:
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['extra_trees_preproc_for_classification', 'liblinear_svc_preprocessor', 'polynomial'],
...
a_file = open("adult_xgb_di_60sp" + str(now) + ".pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_adult_xgb_di_60sp" + str(now) + ".pkl", "wb")
