...
now = now.replace(":", "_")
temp_path = "adult_xgb_eod" + str(now)
try:
...
class CustomXGBoost(AutoSklearnClassificationAlgorithm):
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter("n_estimators", 252, 832, default_value=252)
        max_depth = UniformIntegerHyperparameter("max_depth", 3, 9,
                                                 default_value=6)
        learning_rate = UniformFloatHyperparameter("learning_rate", 0.0744, 0.62224,
                                                   default_value=0.35)
        subsample = UniformFloatHyperparameter("subsample", 0.33998,  0.88498,
                                               default_value=0.88498)
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['select_percentile_classification', 'liblinear_svc_preprocessor', 'extra_trees_preproc_for_classification'],
...
a_file = open("adult_xgb_eod_60sp" + str(now) + ".pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_adult_xgb_eod_60sp" + str(now) + ".pkl", "wb")
