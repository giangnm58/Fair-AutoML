...
now = now.replace(":","_")
temp_path = "german_svc_aod" + str(now)
try:
...
class CustomSVC(AutoSklearnClassificationAlgorithm):
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        C = UniformFloatHyperparameter("C", 0.06199, 10899, log=True,
                                       default_value=0.85)
        ...
        gamma = UniformFloatHyperparameter("gamma", 3.623464265765924e-05, 2.09810,
                                           log=True, default_value=0.1)
        # TODO this is totally ad-hoc
        coef0 = UniformFloatHyperparameter("coef0", -0.24177, 0.73406, default_value=0.0)
        # probability is no hyperparameter, but an argument to the SVM algo
        shrinking = CategoricalHyperparameter("shrinking", ["True", "False"],
                                              default_value="True")
        tol = UniformFloatHyperparameter("tol", 1.25e-5, 0.03287, default_value=1e-3,
                                         log=True)
...
def accuracy(solution, prediction):
    ...
    if os.stat("beta.txt").st_size == 0:
        ...
        default = SVC(C=0.85, break_ties=False, cache_size=200, class_weight=None, coef0=0.0,
            decision_function_shape='ovr', degree=3, gamma='scale', kernel='linear',
            max_iter=-1, random_state=42, shrinking=True, tol=0.001, probability=True,
            verbose=False)
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['extra_trees_preproc_for_classification', 'select_percentile_classification', 'nystroem_sampler'],
...
a_file = open("german_svc_aod" + str(now) + "60sp.pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_german_svc_aod" + str(now) + "60sp.pkl", "wb")
