...
class CustomKNN(AutoSklearnClassificationAlgorithm):
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_neighbors = UniformIntegerHyperparameter(
            name="n_neighbors", lower=12, upper=56, log=True, default_value=12)
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['fast_ica', 'pca', 'select_percentile_classification'],
...
a_file = open("german_knn_spd" + str(now) + "60sp.pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_german_knn_spd" + str(now) + "60sp.pkl", "wb")
