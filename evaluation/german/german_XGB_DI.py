...
class CustomXGBoost(AutoSklearnClassificationAlgorithm):
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter("n_estimators", 227, 754, default_value=227)
        max_depth = UniformIntegerHyperparameter("max_depth", 3, 9,
                                                 default_value=6)
        learning_rate = UniformFloatHyperparameter("learning_rate", 0.31604, 0.77315,
                                                   default_value=0.31604)
        subsample = UniformFloatHyperparameter("subsample", 0.31112, 0.76280,
                                               default_value=0.76280)
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['polynomial', 'extra_trees_preproc_for_classification', 'select_percentile_classification'],
...
a_file = open("german_xgb_di" + str(now) + "60sp.pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_german_xgb_di" + str(now) + "60sp.pkl", "wb")
