...
now = now.replace(":","_")
temp_path = "german_svc_di" + str(now)
try:
...
class CustomSVC(AutoSklearnClassificationAlgorithm):
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        C = UniformFloatHyperparameter("C", 0.03833, 7706, log=True,
                                       default_value=0.85)
        ...
        gamma = UniformFloatHyperparameter("gamma", 3.623464265765924e-05, 1.083,
                                           log=True, default_value=0.1)
        # TODO this is totally ad-hoc
        coef0 = UniformFloatHyperparameter("coef0", -0.40253, 0.51697, default_value=0.0)
        # probability is no hyperparameter, but an argument to the SVM algo
        shrinking = CategoricalHyperparameter("shrinking", ["True", "False"],
                                              default_value="True")
        tol = UniformFloatHyperparameter("tol", 1.0525339408630472e-05, 0.01857, default_value=1e-3,
                                         log=True)
...
automl = autosklearn.classification.AutoSklearnClassifier(
    ...
    ensemble_size=1,
    include_preprocessors=['extra_trees_preproc_for_classification', 'feature_agglomeration', 'kernel_pca'],
...
a_file = open("german_svc_di" + str(now) + "60sp.pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_german_svc_di" + str(now) + "60sp.pkl", "wb")
